...
async function handleInput(pool:Pool,msg:TgMessage){
  ...
  if(s.flow==="{const id=Number(value);if(!Number.isSafeInteger(id))return send(msg.chat.id,"آیدی معتبر نیست.");await pool.query("INSERT INTO bot_blacklist(user_id,reason) VALUES($1,'افزودن دستی مالک') ON CONFLICT(user_id) DO NOTHING",[id]);clearSession(uid);return send(msg.chat.id,"✓ مشتری به لیست سیاه اضافه شد.");}
  ...
  if(s.flow==="confirm_blacklist_remove" && value==="تأیید نهایی"){const id=Number(s.data.id);await pool.query("DELETE FROM bot_blacklist WHERE user_id=$1",[id]);clearSession(uid);return send(msg.chat.id,"✓ مشتری از لیست سیاه خارج شد.");}
  ...
  if(s.flow==="schedule"){const a=s.data.action;if(a==="add"&&Number(s.data.step||1)===1){s.data.message=value;s.data.step=2;session(uid,s.flow,s.data);return send(msg.chat.id,"تاریخ و ساعت ارسال را با قالب YYYY-MM-DD HH:mm ارسال کنید.");}if(a==="add"&&Number(s.data.step||1)===2){const when=new Date(value.replace(" ","T")+":00");if(Number.isNaN(when.getTime())||when.getTime()<=Date.now())return send(msg.chat.id,"تاریخ/ساعت باید معتبر و در آینده باشد.");await pool.query("INSERT INTO bot_schedules(group_id,creator_id,message_text,send_at) VALUES($1,$2,$3,$4)",[groupId,uid,s.data.message,when]);clearSession(uid);return send(msg.chat.id,"✓ زمان‌بندی ثبت شد: "+faDate(when));}const id=Number(value);if(!Number.isSafeInteger(id)){clearSession(uid);return send(msg.chat.id,"شناسه معتبر نیست.");}if(a==="delete")await pool.query("DELETE FROM bot_schedules WHERE id=$1 AND group_id=$2",[id,groupId]);if(a==="toggle")await pool.query("UPDATE bot_schedules SET enabled=NOT enabled WHERE id=$1 AND group_id=$2",[id,groupId]);if(a==="edit"){s.data.scheduleId=id;s.data.step=2;session(uid,s.flow,s.data);return send(msg.chat.id,"متن جدید زمان‌بندی را ارسال کنید.");}if(a==="edit"){await pool.query("UPDATE bot_schedules SET message_text=$1 WHERE id=$2 AND group_id=$3",[value,s.data.scheduleId,groupId]);}clearSession(uid);return send(msg.chat.id,"✓ عملیات زمان‌بندی انجام شد.");}
  return false;
}
