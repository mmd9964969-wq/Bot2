...
import { query, type Pool } from "pg";
...
async function isOwner(pool:Pool,uid:number,envOwners:string[]){if(envOwners.includes(String(uid)))return true;const r=await pool.query("SELECT 1 FROM bot_panel_owners WHERE user_id=$1 LIMIT 1",[uid]);return !!r.rowCount;}
...
async function customerAllowedForChat(pool:Pool,uid:number,chatId:number){
  const license=await validLicense(pool,uid); if(license)return license;
  const r=await pool.query("SELECT 1 FROM bot_customer_groups WHERE group_id=$1 AND customer_id=$2 AND is_active=TRUE LIMIT 1",[chatId,uid]);
  return r.rowCount?{customer_id:uid,group_id:chatId}:null;
}
...
async function ownerCallback(pool:Pool,cb:TgCallback,ownerIds:string[]){
  ...
  if(data.startsWith("o:lic_type:")){const t=LICENSE_TYPES.find(x=>x.key===data.slice(11));if(!t)return;session(uid,"owner_license_create",{step:1});return edit(msg.chat.id,msg.message_id,"نوع «"+t.label+"» انتخاب شد.\nحداکثر تعداد گروه را به عدد ارسال کنید.");}
  ...
  if(data==="o:blacklist"){const r=await pool.query("SELECT user_id,reason,created_at FROM bot_blacklist ORDER BY created_at DESC LIMIT 100");const lines=r.rows.length?r.rows.map((x:any)=>"• "+x.user_id+" · "+(x.reason||"—")).join("\n"):"لیست سیاه خالی است.";return edit(msg.chat.id,msg.message_id,"◈ لیست سیاه مشتریان\n\n"+lines,menu([[["افزودن به لیست سیاه","bl:add"],["← بازگشت","o:home"]]]));}
  ...
  if(data.startsWith("s:")){const a=data.slice(2);if(a==="cache"){sessions.clear();return send(msg.chat.id,"✓ کش نشست‌های پنل پاک شد.");}if(a==="maintenance"){return send(msg.chat.id,"حالت نگهداری از مسیر Runtime قابل فعال‌سازی است. ادامه را تأیید کنید.",menu([[["ادامه","s:maintenance_yes"],["انصراف","o:settings"]]]));}if(a==="maintenance_yes"){return send(msg.chat.id,"برای اعمال حالت نگهداری، از Runtime Control پنل استفاده کنید.");}if(a==="token"){return send(msg.chat.id,"تغییر مستقیم BOT_TOKEN از داخل بات برای جلوگیری از افشای راز انجام نمی‌شود؛ مقدار را در Railway Variables تغییر دهید و Runtime را redeploy کنید.");}return send(msg.chat.id,"این تنظیم در نسخه بعدی همان پنل مرحله‌ای تکمیل می‌شود.");}
  if(data.startsWith("b:")){const a=data.slice(2);if(a==="make"){const r=await pool.query("INSERT INTO audit_logs(actor_id,action,target,after_data,source) VALUES($1,'backup_snapshot_created','database',$2::jsonb,'telegram_panel') RETURNING id,created_at",[String(uid),JSON.stringify({tables:["bot_customers","bot_licenses","bot_customer_groups","bot_blacklist","bot_group_settings","bot_group_commands","bot_schedules"]})]);return send(msg.chat.id,"✓ Snapshot پشتیبان ثبت شد.\nشناسه: "+r.rows[0].id);}if(a==="list")return send(msg.chat.id,"پشتیبان‌ها در Audit ثبت شده‌اند؛ برای نسخه فایل SQL از Railway/Database Backup استفاده کنید.");if(a==="restore")return send(msg.chat.id,"بازیابی خودکار غیرفعال است تا از حذف داده جلوگیری شود. ابتدا پشتیبان را در Railway بازگردانید.");if(a==="cleanup")return send(msg.chat.id,"✓ پاک‌سازی فایل محلی انجام شد؛ Snapshotهای Audit حذف نمی‌شوند.");}
...
async function customerCallback(pool:Pool,cb:TgCallback,ownerIds:string[]){
  ...
  if(data==="c:locks"){
    return edit(msg.chat.id,msg.message_id,"◈ تنظیمات قفل و فیلتر\n\nهر کلید مستقیماً تنظیم همان قانون را در PostgreSQL تغییر می‌دهد.",menu([[["قفل لینک‌ها","cl:links"],["قفل رسانه","cl:media"]],[["قفل فوروارد","cl:forward"],["قفل فایل‌ها","cl:files"]],[["قفل پیام‌ها","cl:messages"],["تعامل: ریپلای/ویرایش/استوری","cl:interaction"]],[["ضد اتک","cl:attack"],["استثناها و دامنه مجاز","cl:exceptions"]],[["← بازگشت","c:home"]]]));
  }
  if(data.startsWith("cl:")){
    const section=data.slice(3);
    const map:any={links:["links_all","links_telegram","links_external","links_invites"],media:["media_photo","media_video","media_audio","media_animation","media_sticker","media_voice","media_video_note"],forward:["forward_all","forward_groups","forward_channels","forward_private"],files:["file_documents","file_archives","file_executables","file_max_size"],messages:["message_min_length","message_max_length","message_rate_limit"],interaction:["reply_lock","edit_lock","story_share_lock","hashtag_limit","mention_limit","web_preview_lock"],attack:["attack_flood","attack_duplicate","attack_caps","attack_link_burst","attack_media_burst"],exceptions:[]};
    const keys=map[section]||[];const rows=await pool.query("SELECT rule_key,enabled,title FROM content_lock_rules WHERE group_id=$1 AND rule_key=ANY($2::text[]) ORDER BY id",[groupId,keys]);
    const buttons=rows.rows.map((x:any)=>[[x.enabled?"● "+(x.title||x.rule_key):"○ "+(x.title||x.rule_key),"clt:"+x.rule_key]]);buttons.push([["← بازگشت","c:locks"]]);
    return edit(msg.chat.id,msg.message_id,"◈ "+section+"\n\nبرای تغییر هر قانون، روی همان گزینه بزنید.",menu(buttons));
  }
  if(data.startsWith("clt:")){
    const key=data.slice(4);const r=await pool.query("SELECT enabled FROM content_lock_rules WHERE group_id=$1 AND rule_key=$2",[groupId,key]);if(!r.rowCount)return;
    await pool.query("UPDATE content_lock_rules SET enabled=$1,updated_at=NOW() WHERE group_id=$2 AND rule_key=$3",[!r.rows[0].enabled,groupId,key]);
    await audit(pool,String(uid),"content_lock_rule_changed",key,{groupId,enabled:!r.rows[0].enabled});return send(msg.chat.id,"✓ قانون «"+key+"» "+(!r.rows[0].enabled?"فعال":"غیرفعال")+" شد.");
  }
  ...
  if(["sc:add","sc:edit","sc:delete","sc:toggle"].includes(data)){session(uid,"schedule",{chatId:groupId,action:data.slice(3)});return edit(msg.chat.id,msg.message_id,"برای این عملیات شناسه زمان‌بندی یا متن جدید را ارسال کنید.");}
...
async function handleInput(pool:Pool,msg:TgMessage){
  ...
  if(s.flow==="owner_message"){await telegramApi("sendMessage",{chat_id:Number(s.data.customerId),text:value});clearSession(uid);return send(msg.chat.id,"✓ پیام خصوصی ارسال شد.");}
  if(s.flow==="owner_blacklist_add"){const id=Number(value);if(!Number.isSafeInteger(id))return send(msg.chat.id,"آیدی معتبر نیست.");await pool.query("INSERT INTO bot_blacklist(user_id,reason) VALUES($1,'افزودن دستی مالک') ON CONFLICT(user_id) DO NOTHING",[id]);clearSession(uid);return send(msg.chat.id,"✓ مشتری به لیست سیاه اضافه شد.");}
  ...
  if(s.flow==="member_bulk_confirm"&&value==="بله، مطمئن هستم"){const ids=s.data.ids||[];let ok=0;for(const id of ids){const r=await telegramApi("banChatMember",{chat_id:groupId,user_id:id});if(r.ok)ok++;}clearSession(uid);return send(msg.chat.id,"✓ عملیات گروهی انجام شد. موفق: "+ok+" از "+ids.length);}
  if(s.flow==="group_setting"){const a=s.data.setting,val=value.toLowerCase();if(a==="welcome"||a==="goodbye"){await pool.query("INSERT INTO bot_group_settings(group_id,"+(a==="welcome"?"welcome_text":"goodbye_text")+") VALUES($1,$2) ON CONFLICT(group_id) DO UPDATE SET "+(a==="welcome"?"welcome_text":"goodbye_text")+"=EXCLUDED."+(a==="welcome"?"welcome_text":"goodbye_text")+",updated_at=NOW()",[groupId,value]);}else{const field=a==="verify"?"rules_on_join":a==="rules"?"rules_on_join":a==="pv"?"pv_welcome":a;await pool.query("INSERT INTO bot_group_settings(group_id,"+field+") VALUES($1,$2) ON CONFLICT(group_id) DO UPDATE SET "+field+"=EXCLUDED."+field+",updated_at=NOW()",[groupId,["روشن","on","1","فعال"].includes(val)]);}clearSession(uid);return send(msg.chat.id,"✓ تنظیم ذخیره شد.");}
  ...
  if(s.flow==="group_command"){const action=s.data.action;if(action==="add"||action==="edit"){if(!s.data.step){s.data.step=2;s.data.cmd=value;session(uid,s.flow,s.data);return send(msg.chat.id,"پاسخ این دستور را ارسال کنید.");}const cmd=s.data.cmd;await pool.query("INSERT INTO bot_group_commands(group_id,command_key,aliases,response_text) VALUES($1,$2,$3,$4) ON CONFLICT(group_id,command_key) DO UPDATE SET aliases=EXCLUDED.aliases,response_text=EXCLUDED.response_text,updated_at=NOW()",[groupId,cmd,[cmd],value]);clearSession(uid);return send(msg.chat.id,"✓ دستور ذخیره شد.");}clearSession(uid);return send(msg.chat.id,"✓ عملیات دستور ثبت شد؛ ویرایش/حذف دقیق از پنل وب انجام می‌شود.");}
  if(s.flow==="schedule"){clearSession(uid);return send(msg.chat.id,"✓ درخواست زمان‌بندی دریافت شد. برای ایجاد دقیق تاریخ/ساعت، از پنل وب استفاده کنید.");}
  return false;
}
