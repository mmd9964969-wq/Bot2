...
import { ensureAdvancedPanelSchema, handleAdvancedCustomerCallback, handleAdvancedCustomerInput, handleAdvancedOwnerCallback } from "../src/lib/bot/advanced-panel.ts";
import {
...
const K={
  ownerMain:[[["آمار کلی سیستم","o:stats"],["مدیریت مشتریان","o:customers"]],[["مدیریت لایسنس‌ها","o:licenses"],["لاگ‌های مهم","o:logs"]],[["مرکز گروه‌ها","o:groups"],["پیام همگانی","o:broadcast"]],[["مرکز Runtime","o:runtime"],["تحلیل سیستم","o:analytics"]],[["ممیزی سیستم","o:audit"],["دسترسی مالک‌ها","o:permissions"]],[["مرکز قابلیت‌ها","o:features"],["مرکز AI","o:ai"]],[["پشتیبان‌گیری و بازیابی","o:backup"],["وضعیت سرور و منابع","o:server"]],[["لیست سیاه مشتریان","o:blacklist"],["تنظیمات پیشرفته","o:settings"]],[["خروج از پنل مالک","o:exit"]]],
  customerMain:[[["مرکز کنترل گروه","c:overview"],["وضعیت گروه","c:status"]],[["قفل و فیلتر","c:locks"],["استودیو محتوا","c:content"]],[["مرکز امنیت","c:security"],["اخطار و جریمه","c:warnings"]],[["مدیریت اعضا","c:members"],["خوش‌آمدگویی و خروج","c:welcome"]],[["Command Studio","c:commands"],["Automation Center","c:automation"]],[["Scheduler","c:schedule"],["Analytics Center","c:analytics"]],[["Exception Center","c:exceptions"],["Permission Center","c:permissions"]],[["Bot Health","c:health"],["Audit Center","c:audit"]],[["پشتیبانی و راهنما","c:support"]],[["خروج از پنل","c:exit"]]]
};
...
async function customerCallback(pool:Pool,cb:TgCallback,ownerIds:string[]){
  ...
  const advancedHandled=await handleAdvancedCustomerCallback(pool,cb,ownerIds,groupId); if(advancedHandled)return true;
...
async function handleInput(pool:Pool,msg:TgMessage){
  ...
  if(await handleAdvancedCustomerInput(pool,msg))return true;const uid=msg.from.id,s=getSession(uid);if(!s||s.expires<Date.now())return false;const value=(msg.text||"").trim();const groupId=Number(s.data.chatId||msg.chat.id);
...
export async function dispatchPanelCallback(pool:Pool,cb:TgCallback,ownerIds:string[]){
  ...
  // Otherwise ownerCallback receives c:/cl:/clt:/cls: actions and silently ignores them.
    ...
    const advancedOwner=await handleAdvancedOwnerCallback(pool,cb,ownerIds); if(advancedOwner)return advancedOwner;
