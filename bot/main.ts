...
async function chatAdmins(chatId: number): Promise<Set<number>> {
  ...
  const data = await telegramApi("getChatAdministrators", {
    chat_id: chatId,
  });
...
async function handleMessage(msg: TgMessage) {
  ...
  const adminIds = isPrivate
    ? new Set<number>()
    : await chatAdmins(chat.id);
  ...
  const ctx: BotContext = {
    ...
    chatTitle:
      chat.title ||
      (isPrivate ? msg.from.first_name || "pm" : "chat"),
  ...
  if (
    ...
    (text === "/info" || text.startsWith("/اطلاعات"))
  ) {
    const count = await telegramApi("getChatMemberCount", {
      chat_id: chat.id,
    });

    if (count.ok && typeof count.result === "number") {
      ctx.membersCount = count.result;
    }
  }
  ...
  if (reply.lang) {
    chatLang.set(chat.id, reply.lang);
  }
...
async function poll() {
  ...
  console.log(
    `nizam phase-1 polling as ${config.botName}`,
  );
