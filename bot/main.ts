...
import { ensureAdvancedPanelSchema, advanceAutomationAction, tickSchedules } from "../src/lib/bot/advanced-panel.ts";
...
async function processMessage(msg: TgMessage, edited = false) {
  ...
  if (!edited && studioPool) {
    const automated = await advanceAutomationAction(
      studioPool,
      chat.id,
      msg.from.id,
      msg.from.first_name,
...
async function handleMessage(msg:TgMessage,edited=false){
  ...
  try{await current;}
  finally{if(groupMessageQueues.get(groupId)===tail)groupMessageQueues.delete(groupId);}
}
...
async function poll() {
  ...
  if (studioPool) {
    ...
    await ensureAdvancedPanelSchema(studioPool);
  }
  setInterval(() => void refreshStudio(), 5000);
  setInterval(() => {
    if (studioPool) void tickSchedules(studioPool).catch((error) => console.error("[scheduler]", error));
  }, 5000);
