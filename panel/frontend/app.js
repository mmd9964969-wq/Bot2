...
async function installationPage(){
  shell("نصب و سیاست پاسخ","INSTALLATION GATE · RESPONSE POLICY",
    "<div class=\"installation-hero panel-card\"><div><span class=\"eyebrow\">INSTALLATION GATE · CAPABILITY 14</span><h2>مرکز نصب و سیاست پاسخ</h2><p>نصب گروه، سیاست پاسخ اعضا، پردازش دستورات، اتوماسیون و امنیت از یک نقطه مدیریت می‌شود.</p></div><div class=\"installation-hard\"><span>●</span><b>Private Chat · Disabled</b><small>قابل تغییر نیست</small></div></div>"+
    "<div class=\"installation-layout\"><section class=\"panel-card installation-list-card\"><div class=\"card-head\"><div><span class=\"eyebrow\">GROUPS</span><h3>گروه‌ها</h3></div><button class=\"ghost\" id=\"installationRefresh\">⟳ بررسی مجدد</button></div><div id=\"installationGroups\" class=\"installation-groups\"><div class=\"loading-state\">در حال دریافت...</div></div></section><section class=\"panel-card installation-detail-card\" id=\"installationDetail\"><div class=\"empty-table\">یک گروه را برای مدیریت انتخاب کنید.</div></section></div>"
  );
  const groupsBox=document.getElementById("installationGroups"),detail=document.getElementById("installationDetail");let groups=[],selected=null;
  function renderGroups(){groupsBox.innerHTML=groups.map(function(g){return "<button class=\"installation-group "+(String(g.group_id)===String(selected)?"active":"")+"\" data-group=\""+esc(g.group_id)+"\"><span class=\"installation-group-mark\">"+(g.installed?"●":"○")+"</span><span><b>"+esc(g.title||"بدون نام")+"</b><small>"+esc(g.group_id)+" · "+esc(g.type||"group")+"</small></span><em class=\""+(g.installed?"on":"off")+"\">"+(g.installed?"● INSTALLED":"○ READY")+"</em></button>"}).join("")||"<div class=\"empty-table\">هنوز گروهی در رجیستری ثبت نشده است.</div>";groupsBox.querySelectorAll("[data-group]").forEach(function(b){b.onclick=function(){selectGroup(b.dataset.group)}});}
  function detailHtml(i){return "<div class=\"installation-detail-head\"><div><span class=\"eyebrow\">GROUP INSTALLATION</span><h2>"+esc(i.title||"بدون نام")+"</h2><p>"+esc(i.group_id)+" · "+(i.installed?"● نصب شده":"○ آماده نصب")+"</p></div><span class=\"installation-badge "+(i.installed?"on":"off")+"\">"+(i.installed?"● ACTIVE":"○ READY")+"</span></div><div class=\"installation-meta-grid\"><div><span>⛂ - وضعیت نصب</span><b>"+(i.installed?"فعال":"آماده")+"</b></div><div><span>⛂ - نسخه</span><b>"+esc(i.installation_version||"v1.0.0")+"</b></div><div><span>⛂ - Command</span><b>"+(i.command_policy==="enabled"?"فعال":"خاموش")+"</b></div><div><span>⛂ - حالت دستورات</span><b>Plain · بدون Slash</b></div><div><span>⛂ - Private Chat</span><b>Disabled</b></div><div><span>⛂ - دسترسی ربات</span><b>"+(i.bot_permission_snapshot&&i.bot_permission_snapshot.status==="administrator"?"Administrator":"بررسی شود")+"</b></div></div><div class=\"installation-section-title\"><span class=\"eyebrow\">POLICY</span><h3>سیاست اجرای گروه</h3></div><div class=\"installation-fields\"><label>پاسخ به اعضا<select id=\"instMember\"><option value=\"silent\">Silent</option><option value=\"commands_only\">Commands Only</option><option value=\"automation\">Automation</option><option value=\"custom\">Custom</option></select></label><label>دستورات<select id=\"instCommands\"><option value=\"enabled\">فعال</option><option value=\"disabled\">خاموش</option></select></label><label>امنیت<select id=\"instSecurity\"><option value=\"standard\">Standard</option><option value=\"strict\">Strict</option><option value=\"custom\">Custom</option></select></label></div><div class=\"installation-toggles\"><label class=\"installation-toggle\"><input id=\"instAutomation\" type=\"checkbox\"><span><b>اتوماسیون</b><small>Automation Engine</small></span></label><label class=\"installation-toggle\"><input id=\"instAudit\" type=\"checkbox\"><span><b>ثبت رویدادها</b><small>Audit Log</small></span></label><div class=\"installation-locked\"><span>×</span><div><b>Private Chat</b><small>Always Disabled · Hard Gate</small></div></div></div><div class=\"installation-footer\"><span>⛂ - تغییرات مستقیماً در سیاست گروه ذخیره می‌شوند.</span><button class=\"primary\" id=\"instSave\">✓ ذخیره تنظیمات</button></div>";}
  async function selectGroup(id){selected=id;renderGroups();detail.innerHTML="<div class=\"loading-state\">در حال دریافت جزئیات...</div>";try{const r=await fetch("/api/installations/"+encodeURIComponent(id),{cache:"no-store"}),d=await r.json();if(!r.ok)throw Error(d.error||"API error");detail.innerHTML=detailHtml(d.installation);const i=d.installation;document.getElementById("instMember").value=i.member_message_policy||"silent";document.getElementById("instCommands").value=i.command_policy||"enabled";document.getElementById("instSecurity").value=i.security_mode||"standard";document.getElementById("instAutomation").checked=!!i.automation_enabled;document.getElementById("instAudit").checked=i.audit_enabled!==false;document.getElementById("instSave").onclick=save;}catch(e){detail.innerHTML="<div class=\"loading-state error\">اطلاعات نصب دریافت نشد.</div>";}}
  async function save(){if(!selected)return;const b=document.getElementById("instSave");b.disabled=true;try{const r=await fetch("/api/installations/"+encodeURIComponent(selected),{method:"PUT",headers:{"Content-Type":"application/json"},body:JSON.stringify({actor_id:null,member_message_policy:document.getElementById("instMember").value,command_policy:document.getElementById("instCommands").value,security_mode:document.getElementById("instSecurity").value,automation_enabled:document.getElementById("instAutomation").checked,audit_enabled:document.getElementById("instAudit").checked})}),d=await r.json();if(!r.ok)throw Error(d.error||"API error");await load();await selectGroup(selected);alert("✓ تنظیمات نصب ذخیره شد.");}catch(e){alert(e.message||"ذخیره تنظیمات ناموفق بود.");}finally{b.disabled=false;}}
