...
async function installationsApi(req,res,url){
  if(req.method==="GET"&&url.pathname==="/api/installations"){
    ...
    return res.send(JSON.stringify({installations:r.rows}));
  }
  ...
  if(m&&req.method==="GET"){
    ...
    if(!r.rowCount)return res.status(404).send(JSON.stringify({error:"Group not found"}));
    ...
    return res.send(JSON.stringify({installation:r.rows[0],private_chat:{enabled:false,locked:true},history:h.rows}));
  }
  if(m&&req.method==="PUT"){
    ...
    if(!MEMBER_POLICIES.includes(member)||!COMMAND_POLICIES.includes(command)||!SECURITY_MODES.includes(security))return res.status(400).send(JSON.stringify({error:"Invalid installation policy"}));
    const old=await query("SELECT * FROM bot_group_installations WHERE group_id=$1 LIMIT 1",[m[1]]);
    if(!old.rowCount)return res.status(404).send(JSON.stringify({error:"Installation state not found"}));
    ...
    return res.send(JSON.stringify({installation:saved.rows[0],private_chat:{enabled:false,locked:true}}));
  }
  ...
  if(h&&req.method==="GET"){
    ...
    return res.send(JSON.stringify({events:r.rows}));
  }
  return null;
}
