...
FROM commands c ORDER BY c.id DESC`);
  ...
  if (req.method === "POST" && url.pathname === "/api/commands") {
    ...
    const result=await query("INSERT INTO commands (command_key,fa_name,en_name,enabled,permission_level,required_permission,minimum_role,response_fa,response_en) VALUES ($1,$2,$3,$4,$5,$6,$7,$8,$9) RETURNING *",[String(body.command_key).trim().replace(/^\/+/,"").toLowerCase(),body.fa_name||"",body.en_name||"",body.enabled!==false,Number(body.permission_level??10),requiredPermission,minimumRole,body.response_fa||"",body.response_en||""]);
    const allowed=Array.isArray(body.allowed_roles)?body.allowed_roles.map(r=>String(r).toUpperCase()).filter(r=>roles.includes(r)):roles;
